...
void main() {
    ...
    vec3 fluidCol = fcol.xyz * (1.5 * b + specular * 5.0);
    fluidCol = tanh(fluidCol);

    fragColor = vec4(fluidCol, a);
}
`
...
const FluidSceneComponent = function FluidScene({
    ...
    useFrame((state) => {
        ...
        const nX = Math.max(0, Math.min(1, rawX / size.width))
        const nY = Math.max(0, Math.min(1, 1.0 - (rawY / size.height)))
