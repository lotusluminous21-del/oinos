...
export function ConfirmedFacts({ confirmed, inferred, onCorrect, className, ...props }: ConfirmedFactsProps) {
  ...
  return (
    <div className={cn("skeuo-card p-4 space-y-3", className)} {...props}>
      <h3 className="text-[16px] font-bold text-slate-800 tracking-tight">What I understand</h3>
      ...
      {inferred && inferred.length > 0 && (
        <div className="space-y-2 pt-2 border-t border-slate-200">
          <p className="text-[12px] font-medium text-slate-500 uppercase tracking-wide">Inferred</p>
          {inferred.map((fact, i) => (
            <div key={i} className="flex items-center gap-3">
              <div className="w-[20px] h-[20px] rounded-full bg-amber-400 flex items-center justify-center shrink-0 shadow-[0_2px_4px_rgba(251,191,36,0.3)]">
                <AlertCircle className="w-[12px] h-[12px] text-white" strokeWidth={3} />
              </div>
              <span className="text-[14px] font-semibold text-slate-600 tracking-tight">
                ...
                <span className="text-[11px] text-slate-400 ml-1">(likely)</span>
              </span>
            </div>
          ))}
        </div>
